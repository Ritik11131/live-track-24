...
import { TempAnalysisRepository } from "../../domain/temperatureAnalysis.repository";
...
@Component({
  ...
  styleUrl: './temperature-analysis.component.scss',
  providers:[TempAnalysisRepository]
})
export class TemperatureAnalysisComponent {
  lineChart: any;
  tempData!: number[];
  timestamp!:string[];
  avgTemp!: string;
  ...
  constructor(
    ...
    private tempAnalysisRepository: TempAnalysisRepository,
    private toastService: ToastService
  ) {
    this.subscription = this.vehicleListService.data$.subscribe((data) => {
      if (data !== undefined) {
        this.deviceId = data;
      }
    });
  ...
  ngOnInit(): void {
    ...
    this.getSocReportData(this.deviceId, this.startDate, this.endDate);
  ...
  getSocReportData(deviceId: number, startDate: string, endDate: string) {
    this.dateRange =
    this.datePipe.transform(this.startDate, "yyyy-MM-dd HH:mm:ss") +
    "  --  " +
    this.datePipe.transform(this.endDate, "yyyy-MM-dd HH:mm:ss");
    this.tempAnalysisRepository
      .getTempReportData(deviceId, startDate, endDate)
      .subscribe(
        (data) => {
          this.showData=true;
          const { tempData,timestamp, averageTempValue, maxTempValue, minTempValue    ,startAddress,endAddress } = data;
          this.timestamp= timestamp.map(dateString => 
            this.datePipe.transform(dateString, 'yyyy-MM-dd HH:mm:ss') || ''
          );
          // Assigning values to class properties
          this.tempData = tempData;
          this.avgTemp = averageTempValue;
          this.maxTemp = maxTempValue;
          this.minTemp = minTempValue;
          this.startAddress=startAddress;
          this.endAddress=endAddress
          
          this.getTemperatureAnalysisData(this.tempData,this.timestamp);
        },
  ...
  getTemperatureAnalysisData(tempData: number[],timestamp:string[]) {
              ...
              callbacks: {
                  label: function(context: any) {
                      ...
                      if (context.dataset.label) {
                          label += 'Temp: ' + context.parsed.y.toFixed(2); // Add SOC data value
                      }
                      if (context.parsed.x !== null && timestamp[context.dataIndex]) {
